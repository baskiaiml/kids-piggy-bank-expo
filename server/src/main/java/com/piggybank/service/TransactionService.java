...
public class TransactionService {
    ...
    public Transaction processDeposit(Long userId, TransactionDTO transactionDTO, String createdBy) {
        ...
        BigDecimal investmentAmount = totalAmount.multiply(settings.getInvestmentPercentage()).divide(new BigDecimal("100"));
    ...
    public Transaction processWithdrawal(Long userId, TransactionDTO transactionDTO, String createdBy) {
        ...
        BigDecimal availableBalance = getAvailableBalance(userId, transactionDTO.getKidId(), transactionDTO.getWithdrawalComponent());
        if (availableBalance.compareTo(transactionDTO.getAmount()) < 0) {
            throw new IllegalArgumentException("Insufficient balance in " + transactionDTO.getWithdrawalComponent() + " component");
        }
        ...
        loggingService.logTransaction(userId, transactionDTO.getKidId(), "WITHDRAWAL", transactionDTO.getAmount(), createdBy);
    ...
    public KidDetailsDTO getKidDetails(Long userId, Long kidId) {
        ...
        // Get component balances
        BigDecimal charityBalance = transactionRepository.getAvailableCharityBalance(userId, kidId);
        BigDecimal spendBalance = transactionRepository.getAvailableSpendBalance(userId, kidId);
        BigDecimal savingsBalance = transactionRepository.getAvailableSavingsBalance(userId, kidId);
        BigDecimal investmentBalance = transactionRepository.getAvailableInvestmentBalance(userId, kidId);
        ...
        Page<Transaction> transactions = transactionRepository.findByUserIdAndKidIdOrderByTransactionDateDesc(userId, kidId, pageable);
        ...
        for (Transaction transaction : transactions.getContent()) {
            String component = transaction.getTransactionType() == Transaction.TransactionType.DEPOSIT ? 
                "ALL" : transaction.getWithdrawalComponent().toString();
            
            BigDecimal amount = transaction.getTransactionType() == Transaction.TransactionType.DEPOSIT ? 
                transaction.getTotalAmount() : transaction.getWithdrawalAmount();
    ...
    public BigDecimal getAvailableBalance(Long userId, Long kidId, Transaction.ComponentType component) {
        return switch (component) {
            case CHARITY -> transactionRepository.getAvailableCharityBalance(userId, kidId);
            case SPEND -> transactionRepository.getAvailableSpendBalance(userId, kidId);
            case SAVINGS -> transactionRepository.getAvailableSavingsBalance(userId, kidId);
            case INVESTMENT -> transactionRepository.getAvailableInvestmentBalance(userId, kidId);
        };
    }
    ...
    private void validateWithdrawalLimits(Long userId, Long kidId, Transaction.ComponentType component) {
        ...
        if (component == Transaction.ComponentType.SAVINGS) {
            Long withdrawalsThisMonth = transactionRepository.countWithdrawalsThisMonth(userId, kidId, component, startOfMonth);
            ...
            if (withdrawalsThisMonth >= settings.getInvestmentMonthlyWithdrawalLimit()) {
                throw new IllegalArgumentException("Monthly withdrawal limit reached for Investment component. Limit: " + 
