...
public class TransactionController {
    ...
    public ResponseEntity<?> processDeposit(@RequestHeader("Authorization") String authHeader,
        ...
        try {
            ...
            return ResponseEntity.badRequest()
                    .body(new ApiResponse(false, "Failed to process deposit: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> processWithdrawal(@RequestHeader("Authorization") String authHeader,
        ...
        try {
            ...
            logger.debug("Processing withdrawal for user: {}, kid: {}, component: {}, amount: {}",
                    userId, transactionDTO.getKidId(), transactionDTO.getWithdrawalComponent(),
            ...
            return ResponseEntity.badRequest()
                    .body(new ApiResponse(false, "Failed to process withdrawal: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getKidDetails(@RequestHeader("Authorization") String authHeader,
        ...
        try {
            ...
            return ResponseEntity.badRequest()
                    .body(new ApiResponse(false, "Failed to get kid details: " + e.getMessage()));
        }
    }
    ...
    public ResponseEntity<?> getAllTransactionsForKid(@RequestHeader("Authorization") String authHeader,
            @PathVariable Long kidId) {
        try {
            String token = authHeader.substring(7);
            Long userId = jwtTokenService.getUserIdFromToken(token);

            logger.debug("Getting all transactions for user: {}, kid: {}", userId, kidId);

            List<KidDetailsDTO.TransactionSummaryDTO> allTransactions = transactionService
                    .getAllTransactionsForKid(userId, kidId);

            return ResponseEntity.ok(new ApiResponse(true, "All transactions retrieved successfully", allTransactions));

        } catch (IllegalArgumentException e) {
            logger.warn("Invalid all transactions request: {}", e.getMessage());
            return ResponseEntity.badRequest().body(new ApiResponse(false, e.getMessage()));
        } catch (Exception e) {
            logger.error("Error getting all transactions: {}", e.getMessage());
            return ResponseEntity.badRequest()
                    .body(new ApiResponse(false, "Failed to get all transactions: " + e.getMessage()));
        }
    }
