@echo off
REM deployment/deploy-pipeline.bat
REM Deploys CodePipeline for CI/CD (Windows version)
...
set GITHUB_REPO=%2
set GITHUB_TOKEN=%3
set ECR_REPOSITORY_URI=%4
set ECS_CLUSTER_NAME=%5
set DEV_SERVICE_NAME=%6
set PROD_SERVICE_NAME=%7
...
if %errorlevel% equ 0 (
    ...
    REM Get pipeline name
    for /f %%i in ('aws cloudformation describe-stacks --stack-name %STACK_NAME% --region %REGION% --query "Stacks[0].Outputs[?OutputKey=='PipelineName'].OutputValue" --output text') do set PIPELINE_NAME=%%i
